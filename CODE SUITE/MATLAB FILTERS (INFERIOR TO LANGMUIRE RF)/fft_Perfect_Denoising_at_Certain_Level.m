...
% Добавление шума
f_noise = 100;           % Частота шума (Гц)
noise = 0.5 * sin(2*pi*f_noise*t);  % Шум на 100 Гц
...
% Определение граничной частоты для фильтрации
cutoff_freq = 110;                  % Граничная частота (Гц), выше которой шум
cutoff_index = round(cutoff_freq * N / Fs);  % Индекс частоты в спектре

% Применение фильтрации: удаляем только шум на частоте 100 Гц
filtered_fft = signal_fft;
filtered_fft(cutoff_index:N-cutoff_index) = 0;  % Обнуляем частоты выше cutoff_freq
